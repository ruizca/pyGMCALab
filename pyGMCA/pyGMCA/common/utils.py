...
def m_mad(xin=0):
    ...
    z = (
        np.median(
            abs(xin - np.tile(np.median(xin, axis=1).reshape(nX[0], 1), (1, nX[1]))),
            axis=1,
        )
        / 0.6735
    )
...
def generate_2D_generalized_gaussian(rows, columns, alpha=2):
    ...
    beta = np.sqrt(
        special.gamma(3.0 / alpha) / special.gamma(1.0 / alpha)
    )  # enough to consider r > 0.5
    y = r / beta
...
def MixtMod(n=2, t=1024, sigma1=1, p1=0.2, ptot=0.1):
    ...
    for r in range(0, n):
        p_active = np.int(np.floor(t * ptot * (1.0 - p1)))
...
def Make_Experiment_Coherent(
    ...
    val = np.power(10.0, (-np.linspace(1, n_s - 1, n_s) / (n_s - 1) * dynamic))

    A0 = np.random.randn(n_obs, n_s)
    A0 = np.dot(A0, np.diag(1.0 / np.sqrt(np.sum(A0 * A0, axis=0))))
    ...
    sigma_noise = (
        np.power(10.0, (-noise_level / 20.0))
...
def Make_Experiment_GG(
    n_s=2, n_obs=2, t_samp=1024, noise_level=40, dynamic=0, CondNumber=1, alpha=2
):
    ...
    if CondNumber > 1:
        for r in range(10):
            Uq, Sq, Vq = np.linalg.svd(A0)
            Sq = 1.0 / np.linspace(1, CondNumber, n_s)
...
def Gen_BG_Mixtures(m=2, n=2, t=1024, p=0.1):
    ...
    A = np.dot(A, np.diag(1.0 / np.sqrt(np.sum(A * A, axis=0))))
...
def EvalCriterion(A0, S0, A, S):
    ...
    Q = abs(np.arccos(np.sum(gA0 * gA, axis=0)) * 180.0 / np.pi)
...
def BSSEval(A0, S0, gA, gS):
    ...
    s_target = np.dot(
        np.dot(
            np.diag(1.0 / np.diag(np.dot(S0, S0.T))), np.diag(np.diag(np.dot(S0, S.T)))
        ),
        S0,
    )
    ...
    SDR = 10 * np.log10(
        ...
        / (
            ...
            + np.linalg.norm(e_artif, ord="fro") ** 2
            + np.linalg.norm(e_noise, ord="fro") ** 2
        )
    )
    SIR = 10 * np.log10(
        np.linalg.norm(s_target, ord="fro") ** 2
...
def RecoveredA(MMC, tol=5):
    """
    Reliability measure based on the ability to get a mixing matrix that 
    is tol deg. closer to either the optimal or the best run
    ...
    for q in range(nm[0]):
        for r in range(nm[1]):
            ...
            ToBest[q, r] = np.size(np.where(abs(MMC[q, r, :] - Opt) < tol)) / np.double(
                nm[2]
            )
