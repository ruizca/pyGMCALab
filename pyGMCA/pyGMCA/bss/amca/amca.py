...
def AMCA(
    ...
    """
    Usage:
        Results = AMCA(X,n=2,maxts = 7,q_f=0.1,mints=3,nmax=100,L0=0,UseP=1,verb=0,Init=0,Aposit=False,BlockSize= None,NoiseStd=[],IndNoise=[],Kmax=0.5,AInit=None,tol=1e-6)
    ...
    Results : dict with entries:
            ...
            weights    : t x 1 array (estimated weights)

    Description:
        Computes the sources and the mixing matrix with GMCA

    Example:
        S,A = AMCA(X,n=2,mints=0,nmax=500,UseP=1) will perform GMCA assuming that the data are noiseless
    ...
    Results = {"sources": S, "mixmat": A, "weights": wa}
...
def Core_AMCA(
    ...
    while Go_On:
        ...
        if np.size(indS) > 0:
            ...
            Su = np.dot(
                np.diag(1.0 / np.maximum(1e-9, np.std(S, axis=1))), S
            )  # Normalizing the sources
            wa = (
                1.0
                / n
                * np.power(np.sum(np.power(abs(Su[indS, :]), alpha), axis=0), 1.0 / alpha)
            )
        ...
        Rs = np.dot(
            S[indS, :], Sr[indS, :].T
        )  # It might be better to update non-zero sources
