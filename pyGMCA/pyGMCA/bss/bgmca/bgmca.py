...
from ...common.prox import Prox_SparsePositive as psp
from ...common.utils import mad, randperm


################# bGMCA Main function
def bGMCA(
    X, n, mints=3, nmax=100, L0=0, verb=0, Init=0, blocksize=2, optBlock=0, J=0, optPos=0
):
    """
    Usage:
    ...
    A = np.dot(A, np.diag(1.0 / np.sqrt(np.sum(A ** 2, axis=0))))
    ...
    if J > 0:
        S, A, exception, Sw = bGMCA_MainBlock(
            Xw,
            n,
            A,
            S,
...
################# bGMCA internal code (Weighting the sources only)
def bGMCA_MainBlock(
    X=0,
    n=0,
    A=0,
    ...
    while Go_On:
        ...
        if optBlock == 0:  # Random creation of the block
            if blocksize < n_S[0]:
                IndBatch = randperm(
                    len(indS)
                )  # ---- mini-batch amongst available sources
            else:
            ...
            # Loop over all the sources + take the sources that are the most correlated with the current one
            if blocksize < n_S[0]:
                currSrcInd = it % len(indS)
                currSrcInd = indS[currSrcInd]
                ...
                angTab[
                    currSrcInd
                ] = -2  # 0 : correspond a l'angle le plus grand, 90 degres
        ...
        if np.size(indS) > 0:
            if len(indS) > 1:
                ...
                if (cd_Ra > 1e-12) and not excThisIt:
                    # if the condition number is large enough
                    iRa = np.dot(Va.T, np.dot(np.diag(1.0 / Sa), Ua.T))
                    piA = np.dot(iRa, A[:, indS].T)
                    S[indS, :] = np.dot(piA, Resi)

                if cd_Ra < 1e-12 or excThisIt == 1:
                    # otherwise a few gradient descent steps
                    comptPasGrad += 1
                    print(
                        "GRADIENT STEP INSTEAD OF PSEUDO-INVERSE NUMBER %s" % comptPasGrad
                    )
                    if excThisIt == 0:
                        La = np.max(Sa)
                    else:
                ...
                # only a single element in the selection / the update is straightforward
                S[indS[0], :] = np.dot(
                    1.0 / np.linalg.norm(A[:, indS[0]]) * A[:, indS[0]], Resi
                )
            ...
            for r in range(len(indS)):  # Thresholding
                ...
                Kval = np.min(
                    [np.floor(np.max([0.01, perc * it]) * len(indNZ)), n_S[1] - 1.0]
                )  # Includes an increasing percentage of available entries
                I = abs(St[indNZ]).argsort()[::-1]
                Kval = np.int(np.min([np.max([Kval, n]), len(I) - 1.0]))
            ...
            if len(indS) > 1:
                ...
                A[:, indS] = np.dot(
                    A[:, indS],
                    np.diag(1.0 / (1e-24 + np.sqrt(np.sum(A[:, indS] ** 2, axis=0)))),
                )  # --- Renormalization
            ...
            if verb:
                ...
                deltaTabAngle[it] = np.min(DeltaAngle)
...
def J_alpha(Sin=0, alpha=0.5):
    S = deepcopy(Sin)
    nS = np.shape(S)
    S = np.dot(np.diag(1.0 / (np.max(abs(S), axis=1) + 1e-12)), S)
...
################ PALM internal code
def PALM_NMF_MainBlock(
    X=0, n=0, A=0, S=0, kend=3, nmax=100, L0=0, blocksize=2, tol=1e-12, optPos=0, J=0
):
    """Usage:
    ...
    while Go_On:
        ...
        if np.size(indS) > 0:
            ...
            for r in range(0, np.shape(Stemp)[0]):
                ...
                if thrdTab[indS[r], 0] < 0:
                    # We fix the threshold at the first iteration and keep the same for the following ones.
                    thrd = mad(
                        gradSt
...
################# AMCA Main function
def bGMCA_NMF_ondt_naif(
    X, n, mints=3, nmax=100, L0=0, verb=0, Init=0, blocksize=2, optBlock=0, J=2
):
    """
    Usage:
    ...
    S, A, exception = bGMCA_MainBlock_NMF_ondt_naif(
        Xw,
        n,
        A,
        S,
...
def bGMCA_MainBlock_NMF_ondt_naif(
    X=0,
    n=0,
    A=0,
    S=0,
    ...
    while Go_On:
        ...
        if optBlock == 0:  # Random
            ...
            if blocksize < n_S[0]:
                ...
                angTab = np.dot(A[:, currSrcInd].T, A)
                angTab[currSrcInd] = -2
                # 0 : correspond a l'angle le plus grand, 90 degres
                IndBatch = np.argsort(angTab)
                IndBatch = np.append(IndBatch, currSrcInd)
                IndBatch = IndBatch[::-1]
            else:
        ...
        if np.size(indS) > 0:
            ...
            if optPos in [1, 2]:  # If the non-negativity is enforced on the sources
                ...
                StempMat[:, :, J] = np.dot(np.linalg.pinv(A), Xw[:, :, J])[
                    indS, :
                ]  # coarse scale
...
################ PALM internal code
def PALM_NMF_MainBlock_prox(
    X=0, n=0, A=0, S=0, kend=3, nmax=100, L0=0, verb=0, blocksize=2, tol=1e-12, J=2
):
    """
    Usage:
    ...
    while Go_On:
        ...
        if np.size(indS) > 0:
            ...
            # Thresholding
            Stemp = psp.GFB_WSparse_Positivity_1D(
                Stemp,
                verb=0,
                tol=1e-6,
                niter=250,
                W=thrdTab[indS, :] / specNormAtemp,
                J=J,
                lam=1,
                XisOndt=0,
                A=A[:, indS],
