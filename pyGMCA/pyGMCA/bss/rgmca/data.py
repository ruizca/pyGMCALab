...
def sources(n=8, t=4096, pS=0.05, alphaS=2, ampliS=100):
    ...
    """
    # Support from a Bernoulli law, with activation parameter pS
    S = np.array(stats.bernoulli.rvs(pS, size=(n, t)))
    S[S > 0] = 1

    # Amplitude follows a Generalized Gaussian law, with parameter gS
    S = S * generate2DGeneralizedGaussian(alphaS, n, t)

    # Scale with the ampliS, standard deviation
    return S * ampliS
...
def mixingMatrix(m=16, n=8):
    """
    Generation of the mixing matrix, whose entries are drawn from a 
    Gaussian law. The columns are normalized.
