# -*- coding: utf-8 -*-
"""
This file contains the functions used during the minimization of the beta-divergence. 
These functions are called from the file BetaD_ICA. This implementation is the python 
version of the Matlab code proposed by Neil Gadhok 
...
def deformv2(vf):
    ...
    Input: vf: a n*n vector [v1,v2,..,vnn]
...
def BetaD_Deriv(xf, Wf, uf, Bf, p_i):
    ...
    # Transpose of the inverse of the unmixing matrix
    WTf = (np.linalg.inv(Wf)).T
    ...
    for i in range(0, mf):
        for j in range(0, nf):
            pif[0, j] = pif[0, j] * (
                BetaD_p_i(extraRow(Wf, i).dot(extraCol(xf, j)) - uf[i, 0], p_i[i, 0])
            )
    pif = pif * np.abs(np.linalg.det(Wf))

    # Compute the derivatives  (Assume that bBeta(W) is nul,
    # as proposed in the article of Mihoko and Eguchi)
    Wdf = np.zeros((mf, mf, nf))
    for j in range(0, nf):
        Wdf[:, :, j] = (pif[0, j] ** Bf) * (
            np.eye(mf)
            - (
                (BetaD_h_i(Wf.dot(extraCol(xf, j)) - uf, p_i)).dot(
                    (Wf.dot(extraCol(xf, j))).T
                )
            )
        ).dot(WTf)

    Wdf = (1.0 / nf) * np.sum(Wdf, 2)
    ...
    for j in range(0, nf):
        udf[:, j] = (pif[0, j] ** Bf) * (
            (BetaD_h_i(Wf.dot(extraCol(xf, j)) - uf, p_i))
        ).reshape(mf,)

    u = (1.0 / nf) * np.sum(udf, 1)
...
def BetaD_p_i(z, p_i):
    """ 
    This function takes a matrix and the indicator for the prior on the sources
...
def BetaD_bB(W, B, p_i):
    """ 
    Computes the value of b_beta(W), according to the priors of the sources. 
    This is not used in our implementation: if the sources are assumed to be 
    sparse, it returns zero, the approximation proposed by Mihoko and Eguchi 
    in their paper.
    """
    if p_i == 1:
        d = 0.25
        result = (
            (np.abs(np.linalg.det(W)) ** B)
            * (
                ((2 * (d ** 0.25) * (sp.special.gamma(0.25) ** (-1))) ** B)
                * ((1 + B) ** (-0.25))
            )
            * (1.0 / (B + 1))
        )
    ...
    if p_i == 2:
        # Return zero because there is not easy closed form to use
        result = 0
...
def BetaD_Lbeta(x, W, u, B, p_i):
    ...
    for i in range(0, m):
        for j in range(0, n):
            pif[0, j] = pif[0, j] * (
                BetaD_p_i(extraRow(W, i).dot(extraCol(x, j)) - u[i, 0], p_i[i, 0])
            )

    r_0 = np.abs(np.linalg.det(W)) * pif

    # the value of b_beta is set to zero, as it is proposed by M.Mihoko and
    # S. Eguchi since there is no closed-form for sparse sources.
    L = ((1.0 / n) * (1.0 / B) * np.sum((r_0) ** B)) - ((1.0 - B) / B)
