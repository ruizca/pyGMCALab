...
First PCP is performed on the observations to separate the low-rank term L from
the sparse outliers O. The implementation of PCP follows the suggestions 
presented in the paper 'Robust Principal Component Analysis?' of E. Candes et 
al. JACM 2011. Then GMCA is performed on L to unmix the sources L=AS, 'Sparsity
and morphological diversity in blind source separation' J.Bobin et al. IEEE 
TIP, 2007. The combination is performed for different values of the parameter 
lambda for PCP, and the best estimation of A is returned.
"""
import numpy as np
...
def pcpGMCA(M, Aori, Sori, Aini):
    ...
    while index < 20:
        ...
        Lambda = (np.float(index) / 2.0 + 0.5) / np.sqrt(t)  # current value of lambda

        mu = (
            t * m / (4 * np.sum(np.abs(M)))
        ...
        try:
            ...
            while (
                ite < aS["iteMaxPCP"] and (convL > 1e-3 or convO > 1e-3 or convTot > 1e-6)
            ) or ite < 15:
...
def updateL(mu, M, O, Y):
    ...
    U, s2, UT = np.linalg.svd(
        (M - O + Y * 1.0 / mu).dot((M - O + Y * 1.0 / mu).T), full_matrices=True
    )
    ...
    Sinv = S.copy()
    Sinv[S > 0] = 1.0 / S[S > 0]
    V = Sinv.T.dot(UT.dot(M - O + Y * 1.0 / mu))  # To perform the reconstruction of L
    S = softThres(S, 1.0 / (mu))  # Threshold of the eigenvalue
...
def updateO(Lambda, mu, M, L, Y):
    ...
    """
    return softThres(M - L + Y * 1.0 / mu, Lambda * 1.0 / mu)
