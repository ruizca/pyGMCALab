...
def BetaD_ICA(xori, Aori):
    """
    Minization of the beta-divergence via BFGS. This implementation comes from
    the Matlab implementation proposed by N. Gadhok 
    (http://home.cc.umanitoba.ca/~kinsner/projects/software/index.html) 
    The modifications of the original code are identified as so. In particular,
    this version performs the minimization of the beta-divergence for different 
    values of the beta-divergence, and returns the mixing matrix with the 
    minimal error.
    ...
    # projection matrix, identity if n=m
    Wred = x.dot(np.linalg.pinv((xm)))

    # mean of x (should be close to 0)
    uori = np.mean(x, 1).reshape((dS["n"], 1))

    # initialization of W, which should be close to the inverse of A
    Wori = np.linalg.inv(Wred.dot(Aori))
    ...
    while index < 400:
        ...
        while (
            (
                prev > stopping_criterion
                or (np.abs(fx - fxO) / (np.abs(fxO) + 1e-16) > stopping_criterion)
            )
            and (i < maxiteration_BFGS)
        ) or i < 10:
            # Main loop, which stop if convergence has been reached
            # (decrease in the cost function smaller than stopping-criterion, twice)
            prev = np.abs(fx - fxO) / np.abs(fxO)  # cost function decrease
            fxO = fx  # previous value of the cost function
            if i > 1 and flag:
                # update of the variables (only if the line search was successful
                # (implementation difference with the original implementation))
                v = v1  # Update of v
            ...
            ###
            alpha = 1  # line search coefficient
            sigma = 1.0e-4  # minimal decrease

            alphamin = 1.0e-4  # minimal coefficient for alpha
            ...
            while (
                (fxn > fx + sigma * alpha * pgx)
                and (alpha > alphamin)
                and (i_linesearch < maxiteration_linesearch)
            ):
            ...
            if (
                (fxn > fx + sigma * alpha * pgx)
                or np.sum(np.isnan(W2) == True) > 0
                or np.sum(
                    np.isnan(
                        BetaD_Deriv(x, W, u, B, p_i) - BetaD_Deriv(x, W2, u2, B, p_i)
                    )
                    == True
                )
                > 0
            ):
                # Line search failed. Thus, the BFGS approx of the Hessian has
                # become corrupt, or we are near the minimum (flat valley) and the
                # step size is too big. So restart.
                G = np.eye(m ** 2 + m)
        ...
        for indexC in range(0, dS["n"]):
            ...
            A[:, indexC] = A[:, indexC] / np.linalg.norm(A[:, indexC])

        S = np.sign(np.linalg.pinv(A).dot(xori)) * np.maximum(
            np.abs(np.linalg.pinv(A).dot(xori)) - aS["kSMax"] * dS["ampliN"], 0
