...
def computeSDRmatrix(X, Y):
    ...
    for n in range(0, n_x):
        ...
        # difference between Y and the projection of the 
        # nth column of X projected on each column of Y
        diff = Y - targets

        # norm of diff, by column
        norm_diff_2 = np.maximum(np.sum(diff * diff, 0), np.spacing(1))
        
        # norm of the target, by column
        norm_targets_2 = np.maximum(np.sum(targets * targets, 0), np.spacing(1))
        
        # ratio between the projection and the difference
...
def errors(A, S, Aori, Sori):
    ...
    First reorder the factorization. Then compute Delta_A and the
    number of columns doing an angle smaller than 5 degrees.
    ...
    DeltaA = np.sum(np.abs(np.linalg.pinv(A).dot(Aori) - np.eye(dS["n"]))) / (
        dS["n"] ** 2
    )

    # Compute the angles done between the columns of A and Aori (between 0 and 90):
    angle = np.arccos(np.abs(np.sum(A * Aori, axis=0))) * 180.0 / np.pi
