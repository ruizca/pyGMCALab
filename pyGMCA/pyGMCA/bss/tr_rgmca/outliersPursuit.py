...
First OP is performed on the observations to separate the low-rank term L from
the sparse outliers O (see 'Robust PCA via Outlier Pursuit', H.Xu et al, IEEE
TIT 2012). Then GMCA is performed on L to recover A and S. The combination is 
performed for different values of the parameter lambda for OP, and the best 
estimation of A is returned.
"""
import numpy as np
...
def OP_GMCA(M, Aori, Sori, Aini):
    """
    Implementation of outliers pursuit combined with GMCA. The implementation
    of outliers pursuit, performed for different parameters values, followed 
    the one proposed in 'Robust PCA via Outlier Pursuit', H.Xu et al, IEEE TIT 
    2012. The notations taken are also the ones proposed in the aforementioned
    paper.
    ...
    while flagExit == 0 and index < 50:  # Test for different parameter values
        Lambda = 1.0 / (np.sqrt(dS["t"])) * index / 5.0  # Threshold value
        ...
        try:
            while (k < kMax and convM > 1e-5) or k < 30:  # While have not converge
                ...
                S = softThres(S, mu / 2.0)  # Threshold of the eigenvalues
                L = np.dot(U, np.dot(S, V))

                C_ = C.copy()
                C = Gc * np.maximum(
                    0, 1.0 - Lambda * mu / 2.0 / np.linalg.norm(Gc, axis=0)
                )
                t_ = t
                t = (1.0 + np.sqrt(4 * t_ ** 2 + 1)) / 2.0
                mu = max(nu * mu, muBar)
                k += 1
                convM = np.linalg.norm(L - L_, "fro") / (
                    np.linalg.norm(L_, "fro") + 1e-16
                ) + np.linalg.norm(C - C_, "fro") / (np.linalg.norm(C_, "fro") + 1e-16)

            L = XMCA.Stransf(L)  # DCT/ domain in which the sources are sparse

            if (
                np.linalg.matrix_rank(L) >= dS["n"] and prevRank > 0
            ):  # If the low rank term, L, has a rank larger or equal to n.
                ...
                if deltaGMCA < errorX:
                    ...
                    deltaGMCA > 1.5 * errorX
                ):  # if the error is significantly larger than the best one, stop the algorithm
                    index += 100

                index += 1

            elif (
                np.linalg.matrix_rank(L) == 0
            ):  # If the parameters are not correctly chosen, make a big step
                index += 5
            elif prevRank > 0:  # if we are not too far from the optimum
                index += 1
            elif (
                np.linalg.matrix_rank(L) >= dS["n"] and prevRank == 0
            ):  # if we have made a too large step
                index = max(1, index - 2)
            else:
                index += 1
