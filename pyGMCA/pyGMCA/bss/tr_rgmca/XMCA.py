...
def Stransf(S):
    ...
    Output: n by t matrix, corresponding to the sparse coefficients of S, DCT transform
            of S (orthonormal transformation).
    """
...
def madAxis(xin, axis="none"):
    ...
    if axis == "none":
        ...
        z = (
            np.median(
                abs(xin - np.median(xin, axis=1).reshape((np.shape(xin)[0], 1))), axis=1
            )
            / 0.6735
        )
...
def softThres(x, thres, typeThres):
    ...
    return (
        x * (np.array(np.abs(x) > thres, int))
        - np.sign(x) * thres * np.array(np.abs(x) > thres, int) * typeThres
    )
...
def trRGMCA(X, Aini, Afix=0):
    ...
    Sini = np.linalg.pinv(A).dot(
        X
    )  # First estimated sources- only used to reorder the sources
    ...
    if Afix == 0:  
        # If A is estimated, set the maximal 
        # number of outer loops to 25.
        loopTot = 25
    else:
        # If A is fixed, only O and S are estimated 
        # (and the number of outer loops is set to 1)
        loopTot = 1  
    
    while (I < loopTot and (convA > 2)) or (I < 3 and Afix == 0):
        # Stop the algorithm if the number of outer loops is too large
        # (should not be the case), A is stable, and at least 3 outer loops have been done.
        I += 1
        ...
        if Afix == 0:
            ...
            # run GMCA on Xws - Ows, for estimating A and S
            Sws, A = AMCA((Xws - Ows), A, 0)  

            (Sws, A) = reOrder(A, Sws, Aini, Siniws)  # permutations
            convA = np.max(
                np.arccos(
                    np.abs(np.sum(Aold * A, axis=0))
                    / (np.linalg.norm(Aold, axis=0) * np.linalg.norm(A, axis=0))
                )
                * 180
                / np.pi
            )  # Maximal angle deviation from one estimation of A to another. (Should be smaller than 2 degrees to stop the algorithm).
            ...
            thr_O = (
                ...
                * aS["kOMax"]
                * madO
                * sp.special.gamma(dS["m"] / 2.0 + 0.5)
                / sp.special.gamma(dS["m"] / 2.0)
            )
            if np.max(normO) > thr_O:
                Owo = (Xwo - A.dot(Swo)) * (np.maximum(1.0 - thr_O * WO / normO, 0))
            else:
                O *= 0
                Owo *= 0
        ...
        # Maximal eigenvalue of A.T.A
        L = np.linalg.norm(A.T.dot(A), 2)  

        ## EstimationS of O and S 
        if I == 1:  
            # If this is the first loop, do not reweight
            indexTotW = 1
        else:
            indexTot = aS["Rew"]
        if Afix == 1:  
            # If A is fixed, set the number of reweighting loops to 5
            indexTotW = 5
        else:
            indexTotW = 1

        # criterion for the convergence of the joint O-S estimation 
        # with reweighting (based on the stability of S)
        convW = 1  

        # Reweighting 
        while indexW < indexTotW and (convW > 1e-2 or indexW < 2):  
            # Stop the reweighting if S is stable of the 
            # number of reweighting steps is reached
            S_W = S.copy()  # To check the stability of S

            if indexW == 0:
                # Do not reweight the output of GMCA
                WS = np.ones((np.shape(Sws)))
                WO = np.ones((np.shape(Owo)))
            else:
                # Weights based on the current thresholds and components
                WS = np.maximum(madS * aS["kSMax"], 0) / (
                    madS * aS["kSMax"] + np.abs(Sws)
                )
            ...
            # Estimations O and S
            while index < iteMax and (convO + convS) > 1e-5:
                # Stop the algorithm when O and S are stable (should not reach iteMax)
                if index > iteMax - 2 and (convA <= 1e-4 or Afix == 1):
                    print("Convergence not reached O-S")
                if convO + convS > 1e-4:
                    # Fix the parameters for convergence once the components are stable enough

                    # projected noise level on the sources
                    madS = madAxis(A.T.dot(Stransf(X - A.dot(S) - O)), axis=1)
                    # noise level for the outliers
                    madO = madAxis(Otransf(X - A.dot(S) - O))
                ...
                while convS_sub > 1e-6 and indexS < 5000:
                    ...
                    S = Sback(
                        softThres(
                            Stransf(y + 1.0 / L * A.T.dot(X - A.dot(y) - O)),
                            madS * aS["kSMax"] * 1.0 / L * WS,
                            1,
                        )
                    )
                    t = (1.0 + np.sqrt(1 + 4 * (t_) ** 2)) / 2.0
                    y = S + (t_ - 1.0) / (t) * (S - S_old)
                    t_ = t
...
def palm_trRGMCA(X, Oini, Sini, Aini):
    ...
    if np.linalg.norm(S) > 0:  
        # If the sources are not null: refinement.
        indexW = 1
    else:
        indexW = 0
    ...
    if indexW == 0:
        # If the estimation starts from scratch: reweighting
        iteRew = 3  
    else:
        # do not perform several loops if refinement
        iteRew = 2  
    ...
    while indexW < iteRew and convW > 1e-2:
        ...
        if indexW == 0:
            # if the sources were null, do not update the weights
            WS = np.ones((np.shape(Sws)))
            WO = np.ones((np.shape(Owo)))
        else:
        ...
        ## Palm
        while ((convOS > 1e-5 or convApalm > 1e-4) and iPalm < iteMax) or iPalm < 5:
            # Stop the algorithm when O, S and A are stable
            iPalm += 1
            ...
            if convOS > 1e-4 and iPalm < 50000:
                # Update the parameters whenever we are not stable.
                # The parameters are then fixed for convergence
                madS = madAxis(A.T.dot(Stransf(X - A.dot(S) - O)), axis=1)
                madO = madAxis(Otransf(X - A.dot(S) - O))
            ...
            S = Sback(
                softThres(
                    Stransf(S + 1.0 / L * A.T.dot(X - A.dot(S) - O)),
                    madS * aS["kSMax"] * 1.0 / L * WS,
                    1,
                )
            )
            ...
            A = A + 1.0 / L * (X - O - A.dot(S)).dot(S.T)
            A = A / np.maximum(1, np.linalg.norm(A, axis=0))

            # Metrics for convergence
            convApalm = np.sum(np.abs(np.linalg.pinv(A).dot(A_) - np.eye(dS["n"]))) / (
                dS["n"] ** 2
            )
            convOS = np.linalg.norm(S - S_, "fro") / np.linalg.norm(
                S_, "fro"
            ) + np.linalg.norm(O - O_, "fro") / np.linalg.norm(O_, "fro")
...
def AMCA(Xini, Aini, aMCA):
    """
    Perform GMCA or AMCA (sparse BSS) on the observations Xini, J.Bobin et al., 
    Sparsity and Adaptivity for the Blind Separation of Partially Correlated Sources.
        
    Inputs:
    - Xini: the observations, m by t matrix. Xini corresponds to the observations 
            expressed in a dictionary in which the sources to be estimated are sparse.
    ...
    W = 1.0 / np.linalg.norm(X + 1e-10, axis=0, ord=1) ** 2
    W /= np.max(W)
    ...
    kIni = 10.0  # Starting value for the threshold, k-mad
    ...
    while it < nmax:  # Stop when the maximal number of iterations is reached
        ...
        if np.size(indS) > 0:
            ...
            if cd_Ra > 1e-5:
                # If A has a moderate condition number, performs the
                ...
                # If A has a large condition number, update S with an
                # 'incomplete' gradient descent
                La = np.max(Sa)
            ...
            for r in range(np.size(indS)):
                ...
                # Only consider the entries larger than k-mad
                indNZ = np.where(abs(St) > ((kIni + it * dk) * madAxis(St)))[0]
                if len(indNZ) < dS["n"]:
                    indNZ = np.where(
                        abs(St)
                        >= np.percentile(
                ...
                Kval = np.min([np.floor(perc * (it) * len(indNZ)), dS["t"] - 1.0])
                I = (abs(St[indNZ])).argsort()[::-1]
                Kval = np.int(min(max(Kval, dS["n"]), len(I) - 1))

                # Threshold based on the percentile of entries larger than k-mad
        ...
        # Weights Update
        if aMCA == 1 and it > 1:
            # Weights update for AMCA
            # p- of the lp norm of the weights
        ...
        if len(indA) > 0:
            ...
            if cd_Rs > 1e-4:
                # if the sources have a fair condition number, use the pseudo-inverse
                piS = np.dot(Sr[indA, :].T, np.linalg.inv(Rs))
                A[:, indA] = np.dot(X, piS)
                A = np.dot(A, np.diag(1.0 / (1e-24 + np.sqrt(np.sum(A * A, axis=0)))))
            else:
                # if the condition number of the projected sources is
                # too large, do an 'interrupted' gradient descent
                Ls = np.max(Ss)
                indexSub = 0
                while indexSub < 250:
...
def MCA(Xini):
    """
    MCA algorithm, J.L. Starck et al, Redundant Multiscale Transforms and their 
    application for Morphological Component Analysis.
    
    Performs the separation between AS and O (Xini=AS+O), sparsely represented in two 
    different dictionaries. For this purpose, the components are transformed with 
    Stransf/Sback, Otransf/Oback.
    ...
    """
    # NB: this implementation can be improved - see for instance 
    # J.Bobin et al, Morphological Component Analysis: An Adaptive Thresholding Strategy.
    ...
    for j in range(dS["m"]):
        # MCA is performed on every channel
        ...
        while index < Max:  # Stop after Max iterations
            ...
            # Only consider the entries of Oloc larger than k-mad
            thrO = aS["kOMax"] * madAxis(Otransf(X - ASloc - Oloc))

            # Only consider the entries of ASloc larger than k-mad
            ...
            if np.linalg.norm(Oloc[np.abs(Oloc) > thrO]) > 0:
                thrO = np.percentile(
                    np.abs(Oloc[np.abs(Oloc) > thrO]), 100 * (1.0 - index / np.float(Max))
                )  # Consider an increasing number of entries, larger than k-mad
            ...
            if np.linalg.norm(ASloc[np.abs(ASloc) > thrS]) > 0:
                thrS = np.percentile(
                    np.abs(ASloc[np.abs(ASloc) > thrS]),
                    100 * (1.0 - index / np.float(Max)),
                )  # Threshold update
        ...
        while index < Max or conv > 1e-6:  # Stop after Max iterations or convergence
            ...
            conv = np.linalg.norm(ASloc - ASloc_) / np.linalg.norm(
                ASloc
            ) + np.linalg.norm(Oloc - Oloc_) / np.linalg.norm(
                Oloc
            )  # convergence metric
