# -*- coding: utf-8 -*-
"""
File containing the methods relative to the metric errors. It has mainly been
inspired by the python code proposed by J. Rapin in the toolbox pyGMCALab, at 
...
def SD(O, S, A, Oori, Sori, Aori):
    """
    Compute the metrics, according to Performance measurement in blind audio 
    source separation, E.Vincent et al. Implementation similar to the one of 
    the toolbox pyGMCALab, at http://www.cosmostat.org/software/gmcalab.
    ...
    if np.sum(np.isnan(S)) == 0:
        ...
        decomposition = {}
        # targets
        decomposition["target S"] = sp.sum(S.T * Sr, 0) * Sr
        
        # Projection of S_i on Sori_i
        # interferences
        ...
        if np.max(np.abs(S)):
            ...
            eps = max(
                np.max(np.abs(norm_num_2)) / 1e6, np.spacing(1)
            )  # Upper bound, can be (should be ?) removed.
...
def errors(O, S, A, Oori, Sori, Aori):
    ...
    return np.hstack(
        (
            SD(O, S, A, Oori, Sori, Aori),
            np.hstack(
                (
                    -10
                    * np.log10(
                        np.linalg.norm(O - Oori, "fro")
                        / (np.linalg.norm(Oori, "fro") + 1e-16)
                    ),
                    np.hstack(
                        (
                            np.sum(np.abs(np.linalg.pinv(A).dot(Aori) - np.eye(dS["n"])))
                            / (dS["n"] ** 2),
                            np.hstack(
                                (
                                    np.max(
                                        np.arccos(np.abs(np.sum(A * Aori, axis=0)))
                                        * 180
                                        / np.pi
                                    ),
                                    np.median(
