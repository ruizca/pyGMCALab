...
from pyGMCA.bss import bgmca as cgb
from pyGMCA.common import utils 
...
def launch_GMCA(
    n_sources=[50],
    blocksize=[1, 2, 3, 4, 5, 7, 10, 13, 16, 18, 20, 28, 36, 45, 50],
    numExp=1,
    N_MC=1,
    sauv=2,
    init=1,
    ...
    # Loop for performing N_MC monte carlo simulations
    for It_MC in range(N_MC):  
        valSeed = numExp * N_MC + It_MC + 1
        np.random.seed(valSeed)
        print("Value of the seed %s" % (valSeed))

        # Loop on the different number of sources to be tested
        for R_t in range(len(n_sources)):  
            n = n_sources[R_t]

            # Generating some random sparse sources with random 
            # mixing matrices in the determined case (m=n)
            if init == 1:
                if dataType == 1:
                    X, X0, A0, S0, N = Make_Experiment_Exact(
                        n_s=n,
                        n_obs=n,
                        t_samp=t_samples,
                        noise_level=SNR,
                    ...
                    X, X0, A0, S0, N = Make_Experiment_GeneralizedGaussian(
                        n_s=n,
                        n_obs=n,
                        t_samp=t_samples,
                        noise_level=120,
                    ...
                    X, X0, A0, S0, N = Make_Experiment_HMR(
                        n_s=n,
                        n_obs=8 * n,
                        t_samp=t_samples,
                        noise_level=SNR,
            ...
            if init == 0:  
                # If we do not create new data matrices but rather use some already saved matrices
                X = sio.loadmat(
                    folderInitSave
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_X_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_verPos%s_J%s"
                    % (sourcesStr, numExp, It_MC, R_t, 0, 1, SNR, 3, J),
                    {"donnees": X},
                )
                X = X["donnees"]
                X0 = sio.loadmat(
                    ...
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_X0_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_verPos%s_J%s"
                    % (sourcesStr, numExp, It_MC, R_t, 0, 1, SNR, 3, J),
                    {"donnees": X0},
                )
                X0 = X0["donnees"]
                A0 = sio.loadmat(
                    ...
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_A0_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_verPos%s_J%s"
                    % (sourcesStr, numExp, It_MC, R_t, 0, 1, SNR, 3, J),
                    {"donnees": A0},
                )
                A0 = A0["donnees"]
                S0 = sio.loadmat(
                    ...
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_S0_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_verPos%s_J%s"
                    % (sourcesStr, numExp, It_MC, R_t, 0, 1, SNR, 3, J),
                    {"donnees": S0},
                )
                S0 = S0["donnees"]
                N = sio.loadmat(
                    ...
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_N_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_verPos%s_J%s"
                    % (sourcesStr, numExp, It_MC, R_t, 0, 1, SNR, 3, J),
                    {"donnees": N},
                )
                N = N["donnees"]

            if sauv == 2 and init == 1:  
                # The data matrices are saved
                sio.savemat(
                    folderInitSave
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_X_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                    % (
                        sourcesStr,
                        numExp,
                        It_MC,
                ...
                sio.savemat(
                    ...
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_X0_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                    % (
                        sourcesStr,
                        numExp,
                        It_MC,
                ...
                sio.savemat(
                    ...
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_A0_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                    % (
                        sourcesStr,
                        numExp,
                        It_MC,
                ...
                sio.savemat(
                    ...
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_S0_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                    % (
                        sourcesStr,
                        numExp,
                        It_MC,
                ...
                sio.savemat(
                    ...
                    + "Data/GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
                    + ptotStr
                    + "_sources%s_N_exp%s_MC%s_sourceN%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                    % (
                        sourcesStr,
                        numExp,
                        It_MC,
            ...
            # Loop on the different block sizes to be tested
            for R_p in range(len(blocksize)):      
                ...
                if minibatch > n:
                    ...
                    if verPos == 0:  
                        # No wavelet and no non-negativity constraint
                        g_S_gmca, g_A_gmca, exception = cgb.bGMCA(
                            X,
                            n,
                            Init=initGMCA,
                            mints=1,
                        ...
                        # Wavelet and non-negativity in the direct domain
                        g_S_gmca, g_A_gmca, exception = cgb.bGMCA_NMF_ondt_naif(
                            X,
                            n,
                            Init=initGMCA,
                            mints=1,
                        ...
                        # Wavelets without non-negativity
                        g_S_gmca, g_A_gmca, exception, Sw = cgb.bGMCA(
                            X,
                            n,
                            Init=initGMCA,
                            mints=1,
                        ...
                        # Non-negativity in the direct domain
                        g_S_gmca, g_A_gmca, exception = cgb.bGMCA(
                            X,
                            n,
                            Init=initGMCA,
                    ...
                    if verPos == 0:  
                        # No wavelet and no non-negativity constraint
                        g_S_gmca, g_A_gmca = cgb.PALM_NMF_MainBlock(
                            X,
                            n,
                            A=cp.deepcopy(A_gmca),
                            S=cp.deepcopy(S_gmca),
                            kend=1,
                            nmax=nitPALM,
                            L0=0,
                            blocksize=minibatch,
                        ...
                        # Wavelet and non-negativity in the direct domain
                        g_S_palm, g_A_gmca = cgb.PALM_NMF_MainBlock_prox(
                            X,
                            n,
                            A=cp.deepcopy(A_gmca),
                            S=cp.deepcopy(S_gmca),
                    ...
                    try:
                        C_hyb[R_p, R_t, It_MC] = EvalCriterion(
                            A0, S0, cp.deepcopy(A_hyb), cp.deepcopy(S_hyb)
                        )
                    ...
                    if sauv > 0:  
                        # We save the A and S matrices
                        sio.savemat(
                            folderInitSave
                            + "MatricesAS/GMCA_cond%s_%sech_%sit_ptot"
                            % (cdStr, t_samples, nitGMCA)
                            + ptotStr
                            + "_sources%s_S_gmca_exp%s_MC%s_sourceN%s_blocksizeNumber%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                            % (
                                sourcesStr,
                                numExp,
                                It_MC,
                        ...
                        sio.savemat(
                            ...
                            % (cdStr, t_samples, nitGMCA)
                            + ptotStr
                            + "_sources%s_A_gmca_exp%s_MC%s_sourceN%s_blocksizeNumber%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                            % (
                                sourcesStr,
                                numExp,
                                It_MC,
                        ...
                        sio.savemat(
                            ...
                            % (cdStr, t_samples, nitGMCA)
                            + ptotStr
                            + "_sources%s_S_hyb_exp%s_MC%s_sourceN%s_blocksizeNumber%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                            % (
                                sourcesStr,
                                numExp,
                                It_MC,
                        ...
                        sio.savemat(
                            ...
                            % (cdStr, t_samples, nitGMCA)
                            + ptotStr
                            + "_sources%s_A_hyb_exp%s_MC%s_sourceN%s_blocksizeNumber%s_optBlock%s_init%s_SNR%s_optWhite%s_verPos%s_J%s"
                            % (
                                sourcesStr,
                                numExp,
                                It_MC,
    ...
    if sauv > 0:  
        # The block sizes and the corresponding C_A values are saved
        sio.savemat(
            folderInitSave
            + "GMCA_cluster_GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
            + ptotStr
            + "_sources%s_optBlock%s_blocksize_init%s_SNR%s_optWhite%s_verPos%s_J%s_exp%s"
            % (sourcesStr, optBlock, initGMCA, SNR, 0, verPos, J, numExp),
            {"donnees": blocksize},
        )
        ...
        sio.savemat(
            ...
            + "GMCA_cluster_GMCA_cond%s_%sech_%sit_ptot" % (cdStr, t_samples, nitGMCA)
            + ptotStr
            + "_sources%s_optBlock%s_C_hyb_init%s_SNR%s_optWhite%s_verPos%s_J%s_exp%s"
            % (sourcesStr, optBlock, initGMCA, SNR, 0, verPos, J, numExp),
            {"donnees": C_hyb},
        )
...
def Make_Experiment_Exact(
    n_s=2, n_obs=2, t_samp=1024, noise_level=40, dynamic=10, ptot=0.1, cd=1
):
    ...
    for r in range(0, n_s):
        ind = utils.randperm(t_samp)
        S[r, ind[0 : p_active.astype(int)]] = np.random.randn(p_active.astype(int))

    val = np.power(10, (-np.linspace(1, n_s - 1, n_s) / (n_s - 1) * dynamic))

    A0 = utils.Get_MixmatCond(n_s, n_obs, cd)

    S0 = np.dot(np.diag(1.0 / np.sqrt(np.sum(S * S, axis=1))), S)
    S0 = np.dot(np.diag(val), S0)
    X0 = np.dot(A0, S0)

    N = np.random.randn(n_obs, t_samp)
    sigma_noise = (
        np.power(10, (-noise_level / 20.0))
        * np.linalg.norm(X0, ord="fro")
        / np.linalg.norm(N, ord="fro")
    )
    print(sigma_noise)
    N = sigma_noise * N
...
def Make_Experiment_GeneralizedGaussian(
    n_s=2, n_obs=2, t_samp=1024, noise_level=40, dynamic=10, ptot=0.1, cd=1, alpha=2
):
    S = np.zeros((n_s, t_samp))
    ...
    for r in range(0, n_s):
        ind = utils.randperm(t_samp)
        S[r, ind[0:p_active]] = utils.generate_2D_generalized_gaussian(
            1, p_active.astype(int), alpha
        )
...
def generateAGauss(m, n, standDev, mBarre, nUnderMBarre):
    ...
    for ii in range(nUnderMBarre):
        A[indP1[ii], ii] = 1
        A[:, ii] = np.convolve(
            A[:, ii], sng.gaussian(len(A[:, ii]), standDev), mode="same"
        )
...
def Make_Experiment_HMR(
    n_s=2, n_obs=2, t_samp=1024, noise_level=40, standDev=3, peak_width=0.05, optNorm=1
):
    ...
    compt = 0
    for key in peakList:
        (spectrum, ppm) = snp.get_nmr_spectrum(
            peakList[key], [0, 10], num_samples=t_samp, peak_width=peak_width
        )
        if optNorm == 1:
