...
data_settings = {"rows": 4, "rank": 2, "dB": 20, "width": 8}
...
parameters = {
    ...
    "S_updater": bss.algos.RedWaveUpdater(
        tau_mad=2, redwave_operator=wave, formulation="synthesis"
    ),
    "A_updater": bss.algos.SparseUpdater(tau_mad=0),
    "recording_functions": {"lambda": lambda_rec, "criteria": criteria_rec},
...
updater = bss.algos.RedWaveUpdater(
    tau_mad=2, redwave_operator=wave, formulation="synthesis"
)
...
parameters = {
    ...
    "S_updater": bss.algos.RedWaveUpdater(
        tau_mad=2,
        redwave_operator=wave,
        formulation="analysis",
        direct_sparsity=False,
        reweighted_l1=3,
    ),
    "A_updater": bss.algos.SparseUpdater(tau_mad=0, modifier=modifier),
    "recording_functions": {"lambda": lambda_rec, "criteria": criteria_rec},
    "display_function": lambda data: bss.tools.show_images(
        data["factorization"].S, 0, im_shape
    ),
    "display_time": 1,
}
np.random.seed(58)
